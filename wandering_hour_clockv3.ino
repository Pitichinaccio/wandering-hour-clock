//
//  Wandering Hour Clock (with stepper motor)  by Bernd Pfeifer
//   extended version with
//   - sound module
//   - position check using reflective IR sensor
//   - alternative values for WiFi  SSID and password
//   - automatic detection for DST  (experimental, only for timezone GMT and GMT+1 so far)
//
...
const char* attrib_autodst = "autodst";
...
const int sensor_correction = 2;  // correction value if IR sensor istn't placed exactly at 0 minutes
#define LED_BUILTIN 2   /* built-in led */

#define IN1 19
#define IN2 18
#define IN3 5
#define IN4 23  // umgeleitet vor 17 wegen serieller Schnittstelle

#define RXD2 16 // serial port for DFPLayer
#define TXD2 17

#define IRS 21 // IR-Sensor


int stepDelay;      /* minimum delay / step in uSec */
int nstrikes;  // number of strikes for sound
int msteps;  // manual steps for debugging
int sensor;  //variable for IR sensor input
...
bool qsound = false;  // true, wenn 15-Minuten-sound bereits abgespielt
bool hsound = false;  // ... volle-Stunde-sound ....
...
static const char ntpServerName[] = "ptbtime1.ptb.de";   // PTB-Braunschweig
...
bool recalibrated = false; // recalibration via IR-sensor every 12 hours, once it is done value is set true
...
void setupWiFi() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    ...
    retryCount++;
    if (retryCount >= maxRetryCount/2) {  // try to login unsing alternative SSID and password
...
void recalibrate() {
 Serial.println("recalibration to 1:00"); 
 myStepper.setSpeed(maxSpeed/2); 
 sensor = digitalRead(IRS); 
 digitalWrite(LED_BUILTIN, sensor); 
 while (sensor == HIGH) {      // Signal on my reflective sensor is inverted, gets LOW when reflection detected
    myStepper.step(4);
    sensor = digitalRead(IRS);
    digitalWrite(LED_BUILTIN, sensor); 
 }   
...
void settime() {
 //Serial.println("testloop IRS");
 //for (int z=0; z <= 200000; z++){
 // sensor = digitalRead(IRS); 
 // digitalWrite(LED_BUILTIN, sensor); 
 // Serial.println(sensor);
 //}
 Serial.println("setting clock to 1:00"); 
 myStepper.setSpeed(maxSpeed); 
 sensor = digitalRead(IRS); 
 digitalWrite(LED_BUILTIN, sensor); 
 Serial.print(sensor);
 ...
 delay(500);
 Serial.print("moving backwards and forward again for exact position ...");
 myStepper.step(-34);
 myStepper.setSpeed(maxSpeed/4);
 sensor = digitalRead(IRS);
 digitalWrite(LED_BUILTIN, sensor); 
 while (sensor == HIGH) {
    myStepper.step(4);
 ...
 myStepper.step(sensor_correction);  // if IR sensor isn't placed exyctly at 0 minutes this can be corrected here
 delay(3000);
 Serial.println("setting clock to current time");
 handleDialAdjustments(1, 0);
}
...
if (soundOn == true) {
 if (cMinute == 0 && hsound ==false) {                                  // sound output - strikes every full hour
   nstrikes = localTime.tm_hour % 12;
   if (nstrikes == 0) nstrikes = 12;
   for (int y=1; y <= nstrikes; y++) {myDFPlayer.playFolder(1, 1); delay(2000); } 
   hsound = true;
  }  
 ...
 if (cMinute % 15  == 1) qsound = false;  // eine Minute später wieder auf false setzen
  ...
  if (cMinute != pMinute && cMinute >= (pMinute + updateIntervalMinutes) % 60) { /* time for update? - every updateIntervalMinutes minutes */
    ...
    */
    ...
    myStepper.setSpeed(maxSpeed/2);
    if (cHour == 1 && cMinute == 0 && recalibrated == false) recalibrate(); else myStepper.step(cStep);
    myStepper.idle();
    myStepper.setSpeed(maxSpeed);
    if (cMinute == 2) recalibrated = false;  // setting to false again for next recalibration in 12 hours
    return;
  }
...
void handleRoot() {
  ...
  html += "<label for='autoDST>automatic setting for DST</label><input type='checkbox' id='autodst' name='autodst'" + String( autoDST ? "checked" : "") + "><br><br>";
...
void handleDialAdjustments(int iHour, int iMinute) {
  ...
  if (minuteDifference > 360) minuteDifference = -1 * (720 - minuteDifference);
...
void handleFormSetPreferences() {
  if (server.hasArg("hour_offset") && server.hasArg("minute_offset")) {
    ...
    bool tmp_autodst = server.hasArg("autodst");
    ...
    if (tmp_hour_offset >= -14 && tmp_hour_offset <= 12 && tmp_minute_offset >= 0 && tmp_minute_offset <= 59) {
      ...
      Serial.print("automatic setting for DST:");
      Serial.println(tmp_autodst ? "true" : "false");
      ...
      preferences.putBool(attrib_autodst, tmp_autodst);
...
time_t getNtpMinute()
{
  ...
  while (millis() - beginWait < 1500) {
    ...
    if (size >= NTP_PACKET_SIZE) {
      ...
      isDst =  summertime_EU(year(), month(), day(), hour(), 1);    // calculate if it is DST or not
